...
DualShellCommanderConfig dualshellcommander_config;
...
void drawShellInfo(const char *path) {
  // Title
  char version[8];
  sprintf(version, "%X.%02X", DUALSHELLCOMMANDER_VERSION_MAJOR, DUALSHELLCOMMANDER_VERSION_MINOR);
  if (version[3] == '0')
    version[3] = '\0';

  pgf_draw_textf(SHELL_MARGIN_X, SHELL_MARGIN_Y, TITLE_COLOR, "DualShellCommander %s", version);
...
static void initUsb() {
  ...
  if (dualshellcommander_config.usbdevice == USBDEVICE_MODE_MEMORY_CARD) {
  ...
  } else if (dualshellcommander_config.usbdevice == USBDEVICE_MODE_GAME_CARD) {
  ...
  } else if (dualshellcommander_config.usbdevice == USBDEVICE_MODE_SD2VITA) {
    if (checkFileExist("sdstor0:gcd-lp-ign-entire"))
      path = "sdstor0:gcd-lp-ign-entire";
    else
      infoDialog(language_container[MICROSD_NOT_FOUND]);
  } else if (dualshellcommander_config.usbdevice == USBDEVICE_MODE_PSVSD) {
  ...
  usbdevice_modid = startUsb("ux0:DualShellCommander/module/usbdevice.skprx", path, SCE_USBSTOR_VSTOR_TYPE_FAT);
...
static int fileBrowserMenuCtrl() {
  ...
  if (pressed_pad[PAD_SELECT]) {
    if (dualshellcommander_config.select_button == SELECT_BUTTON_MODE_USB && sceKernelGetModel() == SCE_KERNEL_MODEL_VITA) {
      if (is_safe_mode) {
    ...
    } else if (dualshellcommander_config.select_button == SELECT_BUTTON_MODE_FTP || sceKernelGetModel() == SCE_KERNEL_MODEL_VITATV) {
      // Init FTP
      if (!ftpvita_is_initialized()) {
        int res = ftpvita_init(vita_ip, &vita_port);
        if (res < 0) {
  ...
  if (hold_pad[PAD_UP] || hold2_pad[PAD_LEFT_ANALOG_UP]) {
   ...
   // New movement options with L and R triggers
  ...
  if (dir_level > 0) {
    ...
    if (pressed_pad[PAD_CANCEL]) {
      ...
      fileListEmpty(&mark_list);
      dirUp();
      WriteFile(DUALSHELLCOMMANDER_LASTDIR, file_list.path, strlen(file_list.path) + 1);
      refreshFileList();
    }
  }
...
static int shellMain() {
  ...
  ReadFile(DUALSHELLCOMMANDER_LASTDIR, lastdir, sizeof(lastdir));
...
int main(int argc, const char *argv[]) {  
  ...
  // Init DualShellCommander
  initDualShellCommander();
  ...
  if (!dualshellcommander_config.disable_autoupdate) {
  ...
  // Finish DualShellCommander
  finishDualShellCommander();
